...
const chatItem: any = {
  id: 'chat-456',
  name: '홍길동',  // 채팅방 이름인데 1:1 이라서 상대방 닉네임으로
  type: 'direct', // 채팅 타입
  last_message: { // 마지막 메시지 정보(선택사항)
    content: '안녕하세요.',  // 내용
    create_at: '2025-09~~',  // 작성시간
    sender_nickname: '홍길동', // 보낸사람 닉네임
  },
  other_user : { // 상대방 사용자 정보
    id: 'user-1234',
    email: 'test@test.com',
    nickname: '둘리',
    avatar_url?: 'http~',
  },
  unread_count: 5, // 읽지 않은 메시지 수
  updated_at: "2025--9-27" // 마지막 업데이트 시간
};
```
...
- 어던 데이터가 처리후 API 응답시 동일한 형태로 구성
...
- 데이터 변환 및 데이터 가공
- 에러 처리
- Mock 데이터 테스트 후 실제 API 전환 테스트

### 10.1. Mock Data 준비

- `/src/servoces/chat 폴더` 생성
- `/src/servoces/chat/directChatService.ts 파일` 생성
...
const mockUser: ChatUser[] = [
  { id: '1', email: 'user1@a.com', nickname: '김철수', avatar_url: null },
  { id: '2', email: 'user2@a.com', nickname: '고길동', avatar_url: null },
  { id: '3', email: 'user3@a.com', nickname: '홍길동', avatar_url: null },
  { id: '4', email: 'user4@a.com', nickname: '이영희', avatar_url: null },
];
 ...
 * 채팅방 ID를 키로 해서 각 채팅방의 메시지를 그룹화 후 관리
 */
    ...
    async (participantId: string): Promise<string | null> => {
      try {
        ...
        if (response.success && response.data) {
          // 채팅방 새로 고침으로 목록 갱신
          await loadChats();
          return response.data.id; // 새 채팅 ID 를 전달한 이유는 즉시 채팅방 참여
        } else {
